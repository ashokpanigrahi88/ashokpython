...
def diff_in_days(df):
    ...
    df['time_1'], df['time_2'] = df.min(axis=1), df.max(axis=1)
    new_df = pd.DataFrame()
    new_df['difference_days'] = pd.to_numeric(abs((df.time_2 - df.time_1).astype('timedelta64[D]')), downcast='integer')
...
def return_location(df):
    ...
    df0 = df.locations.apply(json.loads).values.tolist()
    df1 = pd.DataFrame.from_records(df0)
    new_df = pd.DataFrame(df1['short_name'])
    return new_df
...
def return_post_codes(df):
    ...
    def retpostcode(l_string):
        return ' | '.join(re.findall(r'\b[A-Z]{1,2}[0-9][A-Z0-9]?\s*[0-9][ABD-HJLNP-UW-Z]{2}\b', l_string))
    new_df = pd.DataFrame()
    new_df['postcodes'] = df.text.apply(lambda x: retpostcode(x))
